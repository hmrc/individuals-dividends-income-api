...
class RetrieveUkDividendsIncomeAnnualSummaryValidatorSpec extends UnitSpec {
  ...
  private implicit val correlationId = "1234"
  ...
  class Test extends MockAppConfig {
    ...
    val validatorFactory                         = new RetrieveUkDividendsIncomeAnnualSummaryValidatorFactory()
    def validator(nino: String, taxYear: String) = validatorFactory.validator(nino, taxYear)
  ...
  "running a validation" should {
    "return no errors" when {
      "a valid request is supplied" in new Test {
        ...
        result shouldBe Right(RetrieveUkDividendsAnnualIncomeSummaryRequest(Nino(validNino), TaxYear.fromMtd(validTaxYear)))
      }
    }

    "return NinoFormatError error" when {
      "an invalid nino is supplied" in new Test {
        val result = validator("BADNINO", validTaxYear).validateAndWrapResult()
    ...
    "return TaxYearFormatError error" when {
      "an invalid tax year is supplied" in new Test {
        val result = validator(validNino, "BADTAXYEAR").validateAndWrapResult()
