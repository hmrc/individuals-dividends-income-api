...
import v1.controllers.validators.CreateAmendDividendsValidator.validateBusinessRules
...
class CreateAmendDividendsValidatorFactory @Inject() (implicit appConfig: AppConfig) {
  ...
  private val resolveNonEmptyJsonObject = new ResolveNonEmptyJsonObject[CreateAmendDividendsRequestBody]()

  def validator(nino: String, taxYear: String, body: JsValue): Validator[CreateAmendDividendsRequest] = {
    new Validator[CreateAmendDividendsRequest] {
      override def validate: Validated[Seq[MtdError], CreateAmendDividendsRequest] = {
        (
          ResolveNino(nino),
          resolveTaxYear(taxYear),
          resolveNonEmptyJsonObject(body)
        ).mapN(CreateAmendDividendsRequest) andThen validateBusinessRules
      }
    }
  }
