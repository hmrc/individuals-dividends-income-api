...
class RetrieveAdditionalDividendsValidator(nino: String, taxYear: String, employmentId: String) extends Validator[RetrieveAdditionalDividendsRequest] {

  private val resolveTaxYear = ResolveTaxYearMinimum(TaxYear.fromMtd("2025-26"))

  override def validate: Validated[Seq[MtdError], RetrieveAdditionalDividendsRequest] =
    (
      ResolveNino(nino),
      resolveTaxYear(taxYear),
      ResolveEmploymentId(employmentId)
    ).mapN(RetrieveAdditionalDividendsRequest)
}
