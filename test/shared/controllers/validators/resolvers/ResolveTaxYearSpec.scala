...
class ResolveTaxYearSpec extends UnitSpec with ResolverSupport {
  ...
  "ResolveTaxYear" should {
    ...
    "return an error" when {
      ...
      "the tax year is bad" in {
        ResolveTaxYear("20177") shouldBe Invalid(List(TaxYearFormatError))
      }
    }
  }
  ...
  "ResolveTaxYearMinimum" should {
    ...
    "return an error" when {
      ...
      "when the tax year is invalid" in {
        val result: Validated[Seq[MtdError], Option[TaxYear]] = resolver(Some("20177"))
